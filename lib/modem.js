...
Modem.prototype.dial = function(sessionid, mybackend, options, callback) {
	...
	// balancer patch
	console.warn(`SETTING mybackend MAYBE? `, mybackend, sessionid)
	if (mybackend) {
		optionsf.headers['mybackend'] = mybackend
		optionsf.headers['cookie'] = 'useserver=' + mybackend + ';'
	} else {
		debugger
	}

	if (sessionid) {
		optionsf.headers['sessionid'] = sessionid
	} else {
		debugger
	}
...
Modem.prototype.buildPayload = function(err, isStream, statusCodes, openStdin, req, res, json, cb) {
	...
	if (statusCodes[res.statusCode] !== true) {
		...
		if (openStdin) {
			...
			cb(null, json, res.headers && res.headers['mybackend'])
		}
	}

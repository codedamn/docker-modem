...
var defaultOpts = function() {
	...
	if (!process.env.DOCKER_HOST) {
		...
		try {
			...
			throw new Error(
				'DOCKER_HOST env variable should be something like tcp://localhost:1234'
			)
		}
...
Modem.prototype.dial = function(cookie, options, callback) {
	...
	if (options.authconfig) {
		optionsf.headers['X-Registry-Auth'] =
			options.authconfig.key ||
			options.authconfig.base64 ||
			Buffer.from(JSON.stringify(options.authconfig)).toString('base64')
	}

	if (options.registryconfig) {
		optionsf.headers['X-Registry-Config'] =
			options.registryconfig.base64 ||
	...
	// balancer patch
	console.warn(`SETTING COOKIE MAYBE? `, cookie)
	if (cookie) {
		optionsf.headers['sessionid'] = cookie
	}
...
Modem.prototype.buildRequest = function(options, context, data, callback) {
	...
	if (context.hijack === true) {
		console.error('HIJACK SHOULD NOT BE USED. UNUSABLE FOR LOAD BALANCER')
	...
	if (
		!context.hijack &&
		!context.openStdin &&
		(typeof data === 'string' || data === undefined || Buffer.isBuffer(data))
	) {
		req.end()
	}
}
...
Modem.prototype.buildPayload = function(err, isStream, statusCodes, openStdin, req, res, json, cb) {
	if (err) return cb(err, null, res && res.headers && res.headers['set-cookie'])
	...
	if (statusCodes[res.statusCode] !== true) {
		getCause(isStream, res, json, function(err, cause) {
			...
			msg.json = json
			cb(msg, null, res && res.headers && res.headers['set-cookie'])
		})
	} else {
		if (openStdin) {
			cb(null, new HttpDuplex(req, res))
		} else if (isStream) {
			cb(null, res, res && res.headers && res.headers['set-cookie'])
		} else {
			cb(null, json, res && res.headers && res.headers['set-cookie'])
		}
	}
...
Modem.prototype.buildQuerystring = function(opts) {
	...
	Object.keys(opts).map(function(key, i) {
		clone[key] =
			opts[key] && typeof opts[key] === 'object' && key !== 't'
